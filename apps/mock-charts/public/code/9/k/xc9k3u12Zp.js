...
// 世界视角点击中国区域或者中国数据点 进入中国视角

// 中国视角里点击省份区域   返回 世界视角
// 中国视角里点击省份数据点 进入 省份视角
...
myChart.on('click', params => {
    ...
    // name 是China, 显示中国
    // name 是省份,  显示世界
    if (params.data && params.data.ispProvince && params.name !== '中国') {
        var name = names[params.name]
    ...
    } else if (names[params.name]) {
        option.geo.map = 'world';
        option.series[0].data = convertData(china)
        myChart.setOption(option);
    } else if(params.name === 'China' || params.name === '中国' || (/[\u4e00-\u9fa5]/).test(params.name) ) {
        //(params.name === 'China' || params.name === '中国' || (/[\u4e00-\u9fa5]/).test(params.name)) 
